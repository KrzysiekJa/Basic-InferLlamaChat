...
class ChatInferenceProvider:
    """Inference provider for Chat Completions API."""
    ...
    async def inference_stream(
        ...
        return StreamingResponse(
            self.stream_generator_chat(response), media_type="text/event-stream"
        )
...
class ChatStreamProvider:
    """Stream provider for Chat Completions API."""

    async def stream_generator(self, response: AsyncGenerator) -> AsyncGenerator[str, None]:
        """Generate streaming output from chat completions API."""
        tokens_count = 0
